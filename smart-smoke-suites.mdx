...
A workspace on [triggers](/triggers/overview) configures a Smart Smoke Suite as a deployment trigger whose action kind is `generativeSuite`. See [Smart Smoke Suites](/triggers/overview#smart-smoke-suites) for the trigger fields, and [PR testing](/deployment-testing/pr-testing) for what tying a deploy to its pull request requires — without that, the trigger skips every deploy.
