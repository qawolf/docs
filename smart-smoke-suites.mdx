...
- **Stable preview environments** for PRs, [reported to QA Wolf](/triggers/report-deployments) and isolated per pull request.
- **A deployment trigger** matching those preview deploys, with a generative action. See [Generative flow selection](/triggers/global-triggers#generative-flow-selection).

Once configured, every new preview deployment automatically starts a targeted test run. Pausing the trigger turns it off, and resuming brings it back unchanged.
