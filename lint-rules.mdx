...
those you can enable QA Wolf's page object model rules, and set the severity of
any rule yourself.

QA Wolf's AI already writes page objects, whichever rules you enable; every
workspace is built on the POM base classes. The rules enforce the conventions
around them, so drift shows up in the editor and to the AI as it works.
...
to any API whose documentation marks it `@deprecated`. That includes the
...
<Note>
  ...
  packages your code imports. While an AI job runs, it does not resolve
  imports from npm packages, so a rule of this kind reports less there.
</Note>
...
- **Options are not supported.** In `"no-redeclare": ["error", { … }]` the
  severity applies, and QA Wolf drops the options after it.
- **`@qawolf/eslint-plugin-pom` is the only plugin supported.**
  Naming any other plugin in `extends` or `plugins` has no effect, and the
  editor tells you QA Wolf skipped it. QA Wolf can support additional plugins —
  ask, and QA Wolf will look at adding the one you need.
...
object model rules. The base rules still run, and the editor explains why QA
Wolf skipped the file.
