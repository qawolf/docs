...
Pass the path from the upload step as `RUN_INPUT_PATH`.
...
<Note>
  The deployment type is a free-form label QA Wolf matches against the trigger configured for your workspace — `staging` and `preview` are typical. It doesn't identify the app as Android or iOS, and there's no separate iOS value. Use the label your QA Wolf representative configured, and omit it if none was.
</Note>
