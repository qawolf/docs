...
Skills are additive. The agent keeps doing what it normally does and applies your conventions on top, preferring yours wherever they differ from its defaults. It selects them by matching the task against each skill's `description`, so a well-written description is what puts your skill to work.
...
## Write a description that gets your skill selected

The description is how the agent decides your skill is relevant, so give it enough to match on. Four moves make a description work:
...
<Steps>
  ...
  <Step title="Say when to read it">
    Describe the situations that should pull the skill in: "Read this before creating or editing any Acme flow." Include the symptoms, too — an error message like `invalid credentials` or `mail is not defined` is often how the agent first meets the problem your skill solves.
  </Step>
...
The agent names the skill it read as it works. If yours never comes up, revise the description first: add the situations that should trigger it and the specific terms your team uses. Selection happens on the description alone, so that's where it's decided.
