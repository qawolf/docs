...
The CLI ships a `qawolf-cli` agent skill covering every command, so a coding agent can drive it directly. See [Local execution](/local-execution/authenticate) for the full path and the [CLI reference](/libraries/cli/api-reference/index) for every command, flag, and exit code.
