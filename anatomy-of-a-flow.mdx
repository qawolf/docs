...
Prefer a separate flow per platform, each importing from its own [`@qawolf/flows` subpath](#import-statement). When one flow has to cover several — log capture, say, where iOS uses `"safariConsole"` and parses JSON while Android uses `"browser"` with logging preferences — put the difference in a helper function so the flow stays linear.
