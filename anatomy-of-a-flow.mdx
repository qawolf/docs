...
**Declarative launch** is the recommended style. Pass `launch: true` to use the default startup, or an options object to customize it. Either way, QA Wolf starts the browser or app before your callback runs, and the callback receives the platform object ready to use.
...
iOS and Android flows take the same shape with a device target, such as `"iOS - iPhone 15 (iOS 26)"` or `"Android - Pixel 9"`.
...
**Explicit launch** is for advanced flows only. When startup depends on runtime logic, call `launch()` inside the callback — see the [Web API Reference](/libraries/flows/api-reference/web) for details. For Electron desktop app testing, see [Testing Electron apps](/electron).
...
<Note>
  ...
  \
  `platform.target` (full API in the [Top-Level Reference](/libraries/flows/api-reference/top-level)) lets a single flow branch on the active platform at runtime. Use it sparingly — prefer separate platform-specific flows, each importing from its own [`@qawolf/flows` subpath](#import-statement), if you find yourself branching often.
</Note>
