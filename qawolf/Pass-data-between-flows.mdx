...
**Execution order** — A consumer will not run until its producer has published. Use [Run Rules](/qawolf/Ordering-flows) to enforce producer-before-consumer ordering.

**Scheduling** — Producers and consumers must be included in the same scheduled run. Target all flows or use a shared tag. See [Scheduling flows](/qawolf/Scheduling-flows).