...
**Elements** — lets you enter a locator or CSS selector to highlight matching elements in the live browser. Useful for verifying selectors before writing assertions.

<Frame>
  ![Element Picker](/images/element-picker.png)
</Frame>

**Details** — shows individual run steps with timing. Useful for identifying slow or hung steps.
...
<Note>
  Run History only shows results from scheduled runs. Manual runs from the editor or Map tab do not appear here. See [Run a flow or suite](#) for a full explanation of run types.
</Note>