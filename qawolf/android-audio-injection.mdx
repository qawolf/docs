...
`passAudioAsMicrophoneInput` accepts `delaySeconds` (wait before playback starts) and `durationSeconds` (cap playback length; omit to play the whole file). See the [Android Device Controls](/qawolf/libraries/flows/api-reference/android-device-reference) for the full signature.
