...
Launch-enabled flows also receive a platform object, which differs by target:
...
<Tip>
  `platform.target` (full API in the [Top-Level Reference](/qawolf/libraries/flows/api-reference/top-level)) lets a single flow branch on the active platform at runtime. Use it sparingly — prefer [separate platform-specific flows](#import-statement) if you find yourself branching often.
</Tip>
