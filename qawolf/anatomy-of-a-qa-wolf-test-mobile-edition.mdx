...
### Write one flow per platform, not one that branches
...
The iOS version is the same, just importing from `@qawolf/flows/ios` and targeting an iOS device.
...
<Note>
  ...
  \
  `platform.target` (full API in the [Top-Level Reference](/qawolf/libraries/flows/api-reference/top-level)) lets a single flow branch on the active platform at runtime. Use it sparingly — prefer [separate platform-specific flows](#the-import-statement) if you find yourself branching often.
</Note>
