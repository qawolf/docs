...
| `@qawolf/flows/cli` | Running setup, teardown, or other tasks that don't need a browser or device |
...
**Configuration** — specifies where the flow runs and how it starts. Pass a plain target string to name the browser, device, or environment, or pass an object with `target` and `launch` to configure startup at the same time. The target string must exactly match one of the supported values or your flow will fail to initialize — see [Target Literals](/qawolf/libraries/flows/api-reference/index#target-literals) for the current list.
...
Pass `launch: true` to use default startup, or pass an options object to customize it. Either way, QA Wolf starts the browser or app before your callback runs and the callback receives the platform object ready to use.
...
For advanced cases where startup depends on runtime logic, you can call `launch()` explicitly inside the callback. See the [Web API Reference](/qawolf/libraries/flows/api-reference/web) for details. For Electron desktop app testing, see [Launch Electron](/qawolf/electron).

## Callback parameters

Every flow callback receives three parameters regardless of platform:

**`inputs`** — values passed into the flow from outside. Use this to read data that another flow in the same run published. Keys are uppercase by convention, e.g. `inputs["EMAIL"]`.
...
<Note>
  `launch()`, `expect`, `device`, and `platform.target` only work while a flow is running inside the QA Wolf runner. Keep them inside the flow callback — not at the module level. Module-level code should be limited to imports, constants, and pure helper functions.
</Note>
