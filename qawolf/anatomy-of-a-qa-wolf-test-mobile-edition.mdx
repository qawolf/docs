...
**Configuration** — specifies where the flow runs and how it starts. Pass a plain target string to name the browser, device, or environment, or pass an object with `target` and `launch` to configure startup simultaneously — see [Target Literals](/qawolf/libraries/flows/api-reference/top-level#target-literals) for the exact matching rule and the current list of values.
