...
`qawolf auth logout` only removes credentials stored by `qawolf auth login` — see [`auth logout`](/qawolf/libraries/cli/api-reference/commands#auth-logout) for exact behavior. To remove a `QAWOLF_API_KEY` credential, unset the variable in your shell instead.
