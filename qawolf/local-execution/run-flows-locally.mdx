...
<Steps>
  <Step>
    Find the environment's ID — see [Pull an environment](/qawolf/local-execution/pull-flows#pull-an-environment).
  </Step>
  <Step>
...
`--workers` controls how many web flows run concurrently. Android flows only run one at a time — see [`--workers`](/qawolf/libraries/cli/api-reference/commands#flows-run-pattern) for the exact rule.
...
`qawolf flows run` exits with `0` when every flow passes, `1` when one or more fail, and `2` when the pattern selects no runnable flow. Pass `--allow-no-match` to exit `0` for an empty selection instead. See [Exit codes](/qawolf/libraries/cli/api-reference/index#exit-codes) for the full list.
