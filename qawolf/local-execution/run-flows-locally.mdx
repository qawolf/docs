...
`qawolf flows run --env <env>` is the recommended path. With one command, the CLI pulls the environment's flows into a local cache, installs the runtime dependencies they need (Playwright browsers, Android system images and AVDs), and runs them.
...
<Steps>
  ...
  <Step>
    ...
    1. pulls the environment's flows into `.qawolf/staging/`
    2. loads the environment's `.env` file
    3. installs the Playwright browsers and Android dependencies the flows need
    4. runs every flow

    Subsequent runs reuse the cache. To pull a fresh copy, see [Pull your team's flows](/qawolf/local-execution/pull-flows).
  </Step>
</Steps>
