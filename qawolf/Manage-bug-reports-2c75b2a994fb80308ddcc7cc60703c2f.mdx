...
- Use the **Open** and **Closed** tabs to switch between active and resolved reports.
  <Frame>
    ![Bug Report List](/images/bug-report-list.png)
  </Frame>
...
<Steps>
  ...
  <Step>
    ...
    <Frame>
      ![Bug Report Drawer](/images/bug-report-drawer.png)
    </Frame>
  </Step>
  <Step>
...
<Note>
  **Flows with open bugs are still included in scheduled runs.**
  ...
  - If the flow has only non-blocking bugs, and at least one flow in the run fails all three attempts, the run ends as _Needs investigation_. The environment’s status will appear with the <Icon icon="circle" iconType="solid" color="goldenrod" /> icon on both the **Flows** and **Runs** tabs.
</Note>
