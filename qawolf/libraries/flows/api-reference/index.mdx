...
<Note>
  - Allowlisted devices are used for the features that don't work after resigning such as universal links via associated domains.
  - Private devices are used for advanced cases where the devices have to be allocated for a single team.
  
  These targets are available for flows that require a signed-in Apple ID or app allowlisting. Contact your team to enable these.
</Note>