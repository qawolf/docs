...
QA Wolf runs iOS flows on a shared device pool, resigning your app during install so it can run unreleased builds. See [iOS Device Pools](/qawolf/libraries/flows/api-reference/ios-device-pools) for how that works and when you need different device access.
