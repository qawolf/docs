...
QA Wolf runs iOS flows on real, physical devices shared across customers. QA Wolf resigns your app during installation so it can install unreleased builds without a per-app provisioning profile, and cleans up data and configuration between runs so each run starts from a known state. For cases the shared pool can't cover, see [iOS Device Pools](/qawolf/libraries/flows/api-reference/ios-device-pools).
