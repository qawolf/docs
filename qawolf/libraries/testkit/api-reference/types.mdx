...
These are the main public types exposed through `@qawolf/testkit` and
...
The current code exposes this through the client ports, but there is no
...
Used by `saveBaselineScreenshot(...)`. Any page or locator-like object that
supports this screenshot method shape can be used.
...
This is the lower-level runner port used to implement
`saveBaselineScreenshot(...)`.
...
These are the environment-specific ports that runners pass to
`createTestkitClient(...)`.
...
If `saveSnapshot` is provided, the created client exposes
`saveBaselineScreenshot(...)`. Otherwise that method is absent.
