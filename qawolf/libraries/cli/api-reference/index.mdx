...
| `2` | `invalidArgs` | A Commander parse error, an unknown subcommand, a bad flag value, or a `flows run` selection that matched no runnable flow — pass `--allow-no-match` to exit `0` instead. |
...
| `6` | `timeout` | A `--follow` reached its `--timeout` before the work it was following settled. |
