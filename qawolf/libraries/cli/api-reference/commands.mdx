...
- `--env <env>` — environment ID or slug. When set, missing flows are pulled before the run.
...
- `--env <env>` — required. The environment to pull from. Accepts an environment UUID or a kebab-case slug.
