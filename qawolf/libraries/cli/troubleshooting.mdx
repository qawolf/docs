...
Cause: The CLI could not find a credential: no API key in the environment, the system keychain, or the local config file, and no browser session.
...
Cause: The CLI could not connect to the platform at `QAWOLF_HOST_URL`.

Check:

- the network can reach `https://app.qawolf.com` (or the URL configured in `QAWOLF_HOST_URL`)
- `QAWOLF_HOST_URL` does not contain a typo or unexpected trailing path
