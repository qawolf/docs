...
The current exported wait type is shared by both wait methods. The sections
below describe which fields each method actually uses. Pass `{}` when you want
...
- it uses `after` when provided, otherwise the time when `mail.inbox(...)` was
  called
