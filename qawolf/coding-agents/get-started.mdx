...
This is a different way to work with QA Wolf than [QA Wolf's in-app AI](/qawolf/test-automation), which runs inside the QA Wolf platform itself, or [the QA Wolf CLI](/qawolf/local-execution/authenticate), a separate command-line tool for authenticating and running flows — no AI involved.
