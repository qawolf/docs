...
This integration is separate from [QA Wolf's in-app AI](/qawolf/Automate-flows-using-AI-2d35b2a994fb8084b524cfc9aca7c358) and from [using the QA Wolf CLI directly](/qawolf/local-execution/authenticate).
...
If the application is on a private network, configure [network access](/qawolf/QA-Wolf-s-static-IPv4-29c5b2a994fb81bf8487d085abf9dc4d) first. An application reachable from your laptop is not necessarily reachable from QA Wolf.

For definitions of workspaces, environments, flows, and runs, see the [glossary](/qawolf/Glossary-29c5b2a994fb806d93f2ce4554a2fcfc).
...
<Steps>
  <Step title="Give the agent a target">
    Start with your application URL and the selected QA Wolf environment. For example:

    ```text
    Onboard the app at https://staging.example.com to QA Wolf in My Environment. Find a useful first flow and show me the outline before creating it.
    ```
  ...
  <Step title="Verify the finished flow">
    ...
    See [Review run results](/qawolf/Interpret-run-results-2d55b2a994fb80199754e4b2d3ce0318) for interpreting test outcomes.
  </Step>
</Steps>
