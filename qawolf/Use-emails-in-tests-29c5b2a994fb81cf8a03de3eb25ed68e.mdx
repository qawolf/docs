...
description: "Use the built-in getInbox function to send, receive, and verify emails during your QA Wolf test flows."
sidebarTitle: "Use emails in tests"
icon: "mail"
---

## Accessing `getInbox` in your flow

`getInbox` is available through `testContext` in your `flow()` callback — it does not need to be imported from a separate package.

You can access it in two ways:


1. Spread `testContext` and destructure from it, which is useful when you need to pass context to helpers:

```text
await flow("My Flow", "Web - Chrome", async ({ test, ...testContext }) => {
  const { getInbox } = testContext;
});
```
2. Destructure it directly from the flow context:

```text
await flow("My Flow", "Web - Chrome", async ({ test, getInbox }) => {
  // getInbox is ready to use
});
```
...
<Tip>
  `getInbox()` returns the following object: `{ emailAddress, sendMessage, waitForMessage, waitForMessages }`.
</Tip>
