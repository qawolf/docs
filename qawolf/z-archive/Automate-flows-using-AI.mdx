...
If you'd prefer to write flows manually, see [Create flows in the IDE](/qawolf/z-archive/Create-a-new-flow).

<Note>
  This page covers AI creation inside QA Wolf. To create flows from Claude Code, Codex, or another coding agent, see [QA Wolf plugin](/qawolf/coding-agents/get-started).
</Note>
...
Both open the AI chat panel in flow mode. Working with the agent one prompt at a time is [guided creation](/qawolf/z-archive/getting-the-most-out-of-automate-2#guided-creation).

The <Icon icon="chevron-down" /> icon next to **Create flow** opens a drawer with **Outline flows**, which starts an outlining session across a whole application at once — see [Coverage Mapping](/qawolf/product-mapping).
...
For manual debugging, see [Debug tests & flows](/qawolf/z-archive/Debug-tests-and-flows).
