...
| A full flow | Run button in the IDE | Editor only | No |
| All flows in environment | Flows list | Runs tab | No |
| Select flows in environment | Select flows in the flows list | Runs tab | No |
| Scheduled run | Automatic | Runs tab<br />Run <br />History | Yes |

<Note>
  Run History in the Flows tab only shows results from scheduled runs. See [Scheduling flows](/qawolf/z-archive/Scheduling-flows) to set up a schedule.
</Note>
...
- On a schedule — see [Scheduling flows](/qawolf/z-archive/Scheduling-flows)
