...
sidebarTitle: "Check run results"
---
...
| --- | --- | --- |
| `outcomeWhenBlockingBugsInOtherWorkflows` | `green` \| `red` | Defaults to `green`. When set to `red`, `greenlight` will be `false` if workflows not executed in this run have blocking bugs. Experimental. |

## Response

| Field | Description |
| --- | --- |
| `greenlight` | `true` when `runStage` is `"completed"` and there are zero blocking bugs. |
