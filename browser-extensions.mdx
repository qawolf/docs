...
<Note>
  Store your extension zip in team storage and reference it via `process.env.TEAM_STORAGE_DIR`. See [Use files in flows](/use-files-in-flows) for instructions. Replace `<extension-id>` with your extension's ID, which you can find at `chrome://extensions` with Developer mode enabled.
</Note>
