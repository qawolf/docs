...
<Note>
  Store your barcode image in team storage and reference it via `process.env.TEAM_STORAGE_DIR`. See [Use files in flows](/use-files-in-flows) for instructions. The image must encode the exact value your test expects.
</Note>
