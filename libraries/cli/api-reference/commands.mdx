...
- `--env <env_id>` — environment ID. When set, the CLI pulls missing flows before the run. See [where to find it](/local-execution/pull-flows#pull-an-environment).
...
- `--env <env_id>` — required. The environment ID to pull from. See [where to find it](/local-execution/pull-flows#pull-an-environment).
...
- `--environment-id <env>` — the id or alias of the environment to run. See [where to find it](/local-execution/pull-flows#pull-an-environment).
