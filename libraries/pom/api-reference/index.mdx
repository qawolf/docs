...
QA Wolf can enforce conventions on the page objects you and its AI write — see
...
[Troubleshooting](/libraries/pom/troubleshooting).
...
By default `this.create("LoginPage")` resolves to `BasePageObject` — the name is
...
install otherwise. The registry detects overrides with an own-property check on
the registered class's prototype, so declare `popupHandlers()` /
`routeInterceptors()` directly on the class you register — it does not pick up
