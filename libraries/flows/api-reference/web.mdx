---
title: "Web"
description: "Reference for @qawolf/flows/web, the entry point for defining web flows with launch behavior, page assertions, and browser context helpers."
sidebarTitle: "Web"
---
...
QA Wolf maps `browserContext: "persistent"` to Playwright's persistent context mode. Pass `userDataDir` with `browserContext: "persistent"` when you want to reuse a profile directory.
...
To assert on the meaning of AI-generated text rather than its exact wording, use `ai.expect(locator).toSatisfy(...)` from `@qawolf/ai/web`. See [Assert on the meaning of generated text](/semantic-assertions).
...
For visual regression assertions with `expect(page).toHaveScreenshot(...)`, see [Web & Mobile web screenshots](/visual-diffing).