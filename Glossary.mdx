...
A **trigger** runs a set of flows when your application deploys, or on a schedule. Triggers belong to the workspace, so one trigger can cover every environment a deployment reaches. The flows are named directly, selected by tag, or chosen by QA Wolf as a Smart Smoke Suite.
...
See [Triggers](/triggers/overview).

## PR testing

**PR testing** runs your flows on every pull request, against its preview deploy, and reports the result as checks on the pull request. It requires a connected GitHub or GitLab integration covering the deploying repository. See [PR testing](/deployment-testing/pr-testing).

## Smart Smoke Suite

A **Smart Smoke Suite** is a set of flows QA Wolf selects from the pull request behind a deploy, rather than a set you name in advance. See [Smart Smoke Suites](/smart-smoke-suites).
