---
title: "QA Wolf MCP"
description: "Install the QA Wolf MCP, connect your workspace, and create your first flow from Claude Code, Codex, or another coding agent."
hidden: true
---
...
This is a different way to work with QA Wolf than [QA Wolf's in-app AI](/test-automation), which runs inside the QA Wolf platform itself, or [the QA Wolf CLI](/local-execution/authenticate), a separate command-line tool for authenticating and running flows — no AI involved.
...
If the application is on a private network, configure [network access](/QA-Wolf-s-static-IPv4) first. An application reachable from your laptop is not necessarily reachable from QA Wolf.

For definitions of workspaces, environments, flows, and runs, see the [glossary](/Glossary).
...
<Steps>
  ...
  <Step title="Verify the finished flow">
    ...
    See [Run status](/Glossary#run-status) for the possible outcomes.
  </Step>
</Steps>
...
For other plugin problems, [report an issue](https://github.com/qawolf/agent-plugins/issues) without credentials or customer test data. Skills installed in your coding agent are separate from [reusable skills stored in your QA Wolf workspace](/reusable-ai-knowledge).
