...
<Note>
  This page describes [legacy triggers](/legacy-triggers/overview). A workspace that has migrated reports deploys through [deployment events or `deployment.reportStatus`](/triggers/report-deployments) instead. See [Migrate from legacy triggers](/triggers/migrate-from-legacy-triggers).
</Note>
