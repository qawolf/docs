...
This page describes the older per-environment trigger model. A workspace on [global triggers](/triggers/overview) tests pull requests with a deployment trigger that matches preview deploys; see [Migrate from legacy triggers](/triggers/migrate-from-legacy-triggers).
