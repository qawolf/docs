...
description: "Choose a trigger kind, decide which flows it runs, and create it from the triggers page or a coding agent."
sidebarTitle: "Set up a trigger"
keywords: "create a trigger, run tests on deploy, nightly test run, cron, deployment trigger setup"
---
...
<Check>
  If your workspace still has unpaused legacy triggers, stop here and read [Migrate from legacy triggers](/triggers/migrate-from-legacy-triggers) first. The first global trigger you create cuts the whole workspace over.
</Check>

You create and edit triggers on your workspace's triggers page, or through the [QA Wolf MCP](/qawolf-mcp) from a coding agent.
...
<Note>
  A connected integration and a pipeline that emits deployments are separate things. An integration can be connected while the pipeline creates no deployment records at all, in which case QA Wolf hears nothing.
</Note>
...
- **Generative selection** — QA Wolf picks the flows from the pull request. Preview deployments only; see [Generative flow selection](/triggers/global-triggers#generative-flow-selection).
...
<Steps>
  <Step title="Create the trigger">
    Give it a name you will recognize later, the conditions above, and the action.
  </Step>
  <Step title="Check the environment name your pipeline sends">
    A name that does not match the environment you intend lands the deploy in a different environment, which the trigger never sees. See [the environment name is the contract](/triggers/report-deployments#the-environment-name-is-the-contract).
  </Step>
  <Step title="Deploy">
    Only a real deploy proves the wiring end to end. Confirm a run appeared for it on the environment's runs page.
  </Step>
</Steps>

<Warning>
  A deployment trigger that has never seen a deployment is configured, not verified. Nothing tells you an environment name or a pattern is wrong until a deploy fails to produce a run.
</Warning>
