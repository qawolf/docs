...
A global trigger says: when this happens, run these flows. The "this" is an occasion — a deployment your pipeline reports, or a point in time. The "these flows" is an action — the flows QA Wolf runs when the occasion arrives.
...
## Global triggers and legacy triggers

Legacy triggers attached to a single environment and came in three kinds: deployment triggers, scheduled triggers, and the pull-request testing button.

<Warning>
  The first unpaused global trigger in a workspace switches that whole workspace off the legacy deployment path. Legacy deployment triggers and pull-request button triggers stop firing immediately while still displaying as active, and legacy scheduled triggers keep firing until you pause them. Migration is all-or-nothing per workspace.
</Warning>

If your workspace still runs legacy triggers, read [Migrate from legacy triggers](/triggers/migrate-from-legacy-triggers) before creating your first global trigger. [Legacy triggers](/legacy-triggers/overview) describes the older model on its own terms.
