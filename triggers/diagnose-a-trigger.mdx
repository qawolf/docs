...
keywords: "trigger did not fire, no run for my deploy, my deploy did not start any tests, no run appeared for my pull request, wrong flows ran, trigger evaluation, troubleshooting"
---
...
Start from what happened, not from an id. A pull request gives its head commit and branch; a release gives its tag's commit; "yesterday's release to staging" gives a branch and a rough time. Any one of branch, commit, environment or time is enough to search the workspace's deployments on.
...
- **`not-a-deployment-trigger`** — the trigger has no deployment conditions, so no deploy can ever fire it. A schedule is the usual case. Deploy-driven runs need a deployment trigger; see [Set up a trigger](/triggers/set-up-a-trigger#choose-what-starts-the-run).
...
- **Deployments arrive but connect to nothing.** Test runs happen, yet no branch or pull request shows on them and nothing posts back to the code host. The tell is deployments consistently missing a commit and a branch even though the pipeline sends them. When no pull request ever shows, check that an integration covers the deploying repository — see [PR testing](/deployment-testing/pr-testing).
