...
<Note>
  A pipeline that reports deploys only through the legacy path is invisible to these records and to global triggers, even while runs may still be happening. That workspace is due a [migration](/triggers/migrate-from-legacy-triggers), not a diagnosis.
</Note>
